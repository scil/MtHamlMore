...
use MtHaml\Snip\Exception\SnipHouseException;
use MtHaml\Snip\Lib;
...
class SnipHouse implements SnipHouseInterface
{
    ...
    public function addUse($file, $bMainFile = false)
    {
        if (empty($file)) {
            return;
        }
    ...
    protected function parseFile($file)
    {
        ...
        $class = '\MtHaml\Snip\Snip\SnipFileParser';
        $f = fopen($file, 'r');
        $firstline = fgets($f);
        //        echo '...'.$file.'...<br>';
        if (preg_match('/^-#.*SnipParser="(?<class>[^"]+)"/', $firstline, $matches)) {
            if (is_subclass_of($matches['class'], '\MtHaml\Snip\SnipFileParserInterface'))
                $class = $matches['class'];
        }
