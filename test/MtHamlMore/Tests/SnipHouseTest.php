...
namespace MtHamlMore\Tests\NodeVisitor;
...
class SnipHouseTest extends \PHPUnit_Extensions_Story_TestCase
{
    ...
    public function runThen(&$world, $action, $arguments)
    {
        switch($action)
        {
            ...
            case "first use":
            {
                $uses=$world['S']->getUses();
                $this->assertEquals(self::unifyPath($arguments[0]), reset($uses) );
            }
            break;
            case "last use":
            {
                $uses= $world['S']->getUses();
                $this->assertEquals(self::unifyPath($arguments[0]), end($uses) );
            }
            break;
            case "main file is ":
            {
