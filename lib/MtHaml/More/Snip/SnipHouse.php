...
namespace MtHaml\More\Snip;

use MtHaml\Exception;
use MtHaml\More\Exception\SnipHouseException;
use MtHaml\More\Lib;
...
class SnipHouse implements SnipHouseInterface
{
    ...
    protected function parseFile($file)
    {
        ...
        $class = '\MtHaml\More\Snip\SnipFileParser';
        $f = fopen($file, 'r');
        $firstline = fgets($f);
        //        echo '...'.$file.'...<br>';
        if (preg_match('/^-#.*SnipParser="(?<class>[^"]+)"/', $firstline, $matches)) {
            if (is_subclass_of($matches['class'], '\MtHaml\More\SnipFileParserInterface'))
                $class = $matches['class'];
        }
